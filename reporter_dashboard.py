...
st.set_page_config(page_title="Reporter Dashboard", page_icon="👤",
...
[data-testid="metric-container"]{background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:14px 16px;}
[data-testid="stMetricValue"]{font-size:1.55rem!important;font-weight:800!important;color:#0f172a!important;}
[data-testid="stMetricLabel"]{font-size:.72rem!important;font-weight:700!important;color:#64748b!important;}
.js-plotly-plot .plotly text{fill:#0f172a!important;}
.js-plotly-plot .plotly .legendtext{fill:#0f172a!important;}
.js-plotly-plot .plotly .xtick text,.js-plotly-plot .plotly .ytick text{fill:#475569!important;}
...
# shared plotly defaults (NO margin / legend here to avoid duplicates)
def B(h=360):
    return dict(
...
bar_h = max(420, n_rep * 48 + 100)   # height scales with number of reporters
...
with t3:
    ...
    fig = px.imshow(heat_data, text_auto=True, aspect="auto",
                    color_continuous_scale=[[0,"#f0f9ff"],[0.01,"#bfdbfe"],[1,"#1d4ed8"]],
                    zmin=0)
    fig.update_traces(textfont=dict(size=11, color="#0f172a"))
    fig.update_layout(**B(heat_h), coloraxis_showscale=False,
...
# ═══════════════════════════════════════════════════════════════════
# SECTION 3 — Status Distribution (full width)
# ═══════════════════════════════════════════════════════════════════
st.markdown('<div class="wcard">', unsafe_allow_html=True)
...
fig.update_layout(**B(bar_h), xaxis_title="Number of Issues", yaxis_title=None,
                  legend=LEG_H)
...
# ═══════════════════════════════════════════════════════════════════
# SECTION 4 — Priority Distribution (full width)
# ═══════════════════════════════════════════════════════════════════
st.markdown('<div class="wcard">', unsafe_allow_html=True)
...
fig.update_layout(**B(360), xaxis_title=None, yaxis_title="Issues Reported",
                  legend=LEG_H)
...
# ═══════════════════════════════════════════════════════════════════
# SECTION 6 — Resolution Rate (full width)
# ═══════════════════════════════════════════════════════════════════
st.markdown('<div class="wcard">', unsafe_allow_html=True)
...
fig = go.Figure(go.Bar(
    ...
    text=res_rank.apply(
        lambda r: f"{r['res_pct']:.0f}%  ({int(r['resolved'])}/{int(r['total'])})", axis=1
    ),
...
chosen = st.selectbox("Pick reporter", rep_df["reporter"].tolist(),
...
with sp2:
    ...
    fig.update_layout(**B(340), xaxis_title=None, yaxis_title="Issues",
...
with fc1:
    srch = st.text_input("s","", placeholder="🔎  Search summary or issue key…",
                         label_visibility="collapsed")
with fc2:
    tf = st.selectbox("tf",["All Types"]+sorted(df["issue_type"].dropna().unique().tolist()),
                      label_visibility="collapsed")
with fc3:
    sf = st.selectbox("sf",["All Statuses"]+sorted(df["status"].dropna().unique().tolist()),
                      label_visibility="collapsed")
with fc4:
    rf = st.selectbox("rf",["All Reporters"]+sorted(df["reporter"].dropna().unique().tolist()),
                      label_visibility="collapsed")
